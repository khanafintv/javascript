...
const map = {
  ...
  init(rowsCount, colsCount) {
    ...
    for (let row = 0; row < rowsCount; row++) {
      ...
      for (let col = 0; col < colsCount; col++) {
        ...
        tr.appendChild(td);
        if (col == 0 || row == 0 || col == config.getColsCount() - 1 || row == config.getRowsCount() - 1) td.style.cssText = 'border: none; background: #2e353d;';
  ...
  render(snakePointsArray, foodPoint, obstaclePoint) {
    ...
    snakePointsArray.forEach((point, index) => {
      ...
      if (snake.direction == 'up' && point.y == 0) point.y = config.getRowsCount() - 1;
      if (snake.direction == 'down' && point.y == config.getRowsCount() - 1) point.y = 0;

      if (snake.direction == 'right' && point.x == config.getColsCount() - 1) point.x = 0;
      if (snake.direction == 'left' && point.x == 0) point.x = config.getColsCount() - 1;

    });

    document.querySelector('#count').innerHTML = '';
    document.querySelector('#count').insertAdjacentHTML('afterbegin', `Ваш счет: ${countGame - 1}`);
...
const game = {
  ...
  getRandomFreeCoordinates() {
    ...
    let cols = this.config.getColsCount() - 2;
    let rows = this.config.getRowsCount() - 2;

    while (true) {
      const rndPoint = {
        x: Math.floor(Math.random(1) * cols) + 1,
        y: Math.floor(Math.random(1) * rows) + 1,
      };
